...
23. **Numéro de Reversement Mobile Money dans le Profil Utilisateur :** Les prestataires (Artisans, Livreurs, Fournisseurs) peuvent renseigner et mettre à jour leur numéro de téléphone Mobile Money (`payment_phone`) et leur opérateur préféré (`preferred_payment_provider`). Ces coordonnées font autorité pour le virement automatisé des fonds débloqués par ProsArtisan.
