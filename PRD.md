...
16. **Cartographie Interactive Territoriale & Suivi Opérationnel 360° :** [COMPLÉTÉ] Module backoffice React 19/Inertia doté d'une carte vectorielle SVG haute fidélité pour les 14 districts/régions de Côte d'Ivoire et d'un zoom métropolitain sur les 13 communes du Grand Abidjan (`AdminTerritoryService`, `IvoryCoastMapSvg`, `CartographyPanel`). Restitution croisée en temps réel des 4 acteurs (clients, artisans avec part de KYC validés, quincailleries agréées J-Code, livreurs), des KPIs de performance opérationnelle (taux de réalisation %, taux de litige %, volume financier séquestre en FCFA, note moyenne), et tableau récapitulatif dynamique avec filtres multi-entités, recherche instantanée et pagination. Accès sécurisé par la capacité `admin.territory.view`.
