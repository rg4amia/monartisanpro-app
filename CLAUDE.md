...
29. **Ni données de démonstration, ni note par défaut** : aucun contrôleur ne préremplit une vue de valeurs inventées — indiscernables des données réelles, elles masquent toute panne de chargement. Une section sans donnée affiche une mention explicite (`SectionEmptyNote`) disant ce qui la remplira, jamais un cadre muet. Une note jamais attribuée vaut `null` et s'affiche « Non évalué » : `COALESCE(AVG(note), 5.0)` décernait 5/5 à des comptes n'ayant servi personne et rendait le tri arbitraire. Les acteurs évalués passent devant les non-évalués, et `ratings_count` distingue « pas encore noté » de « mal noté ».
