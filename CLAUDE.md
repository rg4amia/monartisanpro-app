...
62. **Pré-Diagnostic Multimodal Vidéo & Photo Gemini 3.6 Flash (`missions.diagnostic_media_analysis`)** : analyse IA instantanée d'un problème de chantier ou sinistre à partir de photos (max 5, 15 Mo/photo) et/ou d'une courte vidéo (max 25 Mo, ≤ 30s) avec description textuelle (`POST /api/v1/missions/pre-diagnostic`). Exige un KYC actif (`kyc.verified`) et est assujetti au quota IA utilisateur (`AiMonitoringService::checkUserLimit` incluant `'pre_diagnostic'`, retour HTTP 429 si dépassé). `GeminiService::analyzePreDiagnostic` envoie les payloads multimédias base64 (`inline_data`) et normalise la réponse : synthèse pathologie (`diagnostic_summary`), sévérité (`faible`, `moyen`, `urgent`), corps d'état recommandé (`recommended_trade`), type d'intervention conseillé, indices visuels observés, précautions de sécurité immédiates (`urgency_precautions`), pièces & matériaux probables (`estimated_materials` en FCFA entiers) et estimation budgétaire (`pricing`). L'analyse est persistée dans `missions.diagnostic_media_analysis` (JSON) à la création de la mission pour préparer l'outillage de l'artisan. Sur mobile (`MissionRequestScreen`), la carte `GeminiEstimateCard` affiche directement les alertes de sécurité, les pièces prévues et pré-remplit le corps de métier conseillé. Tests : `PreDiagnosticMediaTest.php`.
63. **Conformité stricte ENUM MariaDB & Rôle Canonique Livreur (`users.role = 'livreur'`)** : en base de production MariaDB 11.8 et MySQL local, `users.role` est restreint à `ENUM('client','artisan','fournisseur','referent','admin','livreur')`. L'insertion de `'driver'` est rejetée en SQL strict (`1265 Data truncated for column 'role'`). Le modèle `User` intègre un mutateur Eloquent `role` (`Attribute::make(set: fn ($v) => $v === 'driver' ? 'livreur' : $v)`) normalisant automatiquement `'driver'` en `'livreur'`. Les requêtes SQL (`AdminService`, `DeliveryTrackingService`, etc.) et les tests de Feature utilisent impérativement `'livreur'`.
