...
- **Non traités** : reçu PDF des retraits livreur ; encaissement réel des commandes de matériaux, toujours simulé à la création.
...
- **Remboursement client après litige — traité le 26/09/2026 (reliquat)** : `refundClientFromDispute` passe par `MobileMoneyPayoutService` (contexte `remboursement_client`). Un versement porte désormais le titulaire du portefeuille prélevé (`source_user_id`) et la ventilation du débit (`debits`, migration `2026_09_26_140000`) : le client reçoit un seul virement, les portefeuilles matériaux et main-d'œuvre de l'artisan ne sont débités qu'au virement réussi. Correctif associé : `getMissionEscrowBalance` ne soustrayait pas les versements non aboutis dans son repli historique (missions sans ledger par mission) — un paiement d'étape échoué sur une telle mission restait libérable une seconde fois. Tests : `ClientRefundPayoutTest` (6, en échec avant correctif), `PayoutsSection.test.tsx` (mention « Prélevé chez »).
