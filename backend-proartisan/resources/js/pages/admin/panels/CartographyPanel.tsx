...
export function CartographyPanel({
    ...
    const [summary, setSummary] = useState<TerritorySummary>(initialSummary);
    ...
    const filteredCommunesList = selectedDistrict
        ? communesList.filter((c) => c.district_slug === selectedDistrict)
        : communesList;
        ...
        <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-[var(--admin-muted)]">
                                    {summary.zone.type === 'national' && "Territoire National • 14 Districts • Échelle Côte d'Ivoire"}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-4">
                ...
                <MetricCard
                    ...
                    value={summary.actors.clients.total.toString()}
                    description={`${summary.actors.clients.with_active_missions} avec mission active`}
                    tone="blue"
                />
                ...
                <MetricCard
                    ...
                    value={summary.actors.artisans.total.toString()}
                    description={`${summary.actors.artisans.kyc_actif} KYC validés (${summary.actors.artisans.kyc_actif_percent}%)`}
                    tone="amber"
                />
                ...
                <MetricCard
                    ...
                    value={summary.actors.fournisseurs.total.toString()}
                    description={`${summary.actors.fournisseurs.agreed} boutiques certifiées J-Code`}
                    tone="green"
                />
                ...
                <MetricCard
                    ...
                    value={summary.actors.livreurs.total.toString()}
                    description={`${summary.actors.livreurs.active_courses} courses en transit`}
                    tone="slate"
                />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-4">
                ...
                <MetricCard
                    ...
                    value={summary.missions.total.toString()}
                    description={`${summary.missions.en_cours} en cours • ${summary.missions.terminee} terminées`}
                    tone="amber"
                />
                ...
                <div className="rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-card-bg)] p-4 shadow-sm flex flex-col justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-2xl font-bold text-emerald-700">
                            {summary.missions.realization_rate}%
                        </p>
                    </div>
                    <div className="mt-3">
                        <div className="h-2 w-full rounded-full bg-emerald-100 overflow-hidden">
                            <div
                                ...
                                style={{ width: `${Math.min(summary.missions.realization_rate, 100)}%` }}
                            />
                        </div>
                        <p className="mt-1.5 text-[11px] text-[var(--admin-muted)]">
                            {summary.missions.terminee} chantiers menés à terme
                        </p>
                    </div>
                </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between">
                            ...
                            <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                                summary.missions.dispute_rate > 5
                                    ? 'bg-rose-100 text-rose-800'
                                    : 'bg-slate-100 text-slate-700'
                            }`}>
                                {summary.missions.litige} litige{summary.missions.litige > 1 ? 's' : ''}
                            </span>
                        </div>
                        <p className={`mt-1 text-2xl font-bold ${
                            summary.missions.dispute_rate > 5 ? 'text-rose-600' : 'text-slate-800'
                        }`}>
                            {summary.missions.dispute_rate}%
                        </p>
                    </div>
                    <div className="mt-3">
                        <div className="h-2 w-full rounded-full bg-slate-100 overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all duration-500 ${
                                    summary.missions.dispute_rate > 5 ? 'bg-rose-500' : 'bg-amber-400'
                                }`}
                                style={{ width: `${Math.min(summary.missions.dispute_rate * 4, 100)}%` }}
                            />
                        </div>
                        <p className="mt-1.5 text-[11px] text-[var(--admin-muted)]">
                            {summary.missions.litige === 0
                ...
                <MetricCard
                    ...
                    value={money(summary.missions.financial_volume_fcfa)}
                    description={`Note artisan moyenne : ${summary.reputation.average_artisan_rating}/5 (${summary.reputation.total_reviews} avis)`}
                    tone="green"
                />
            </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-[var(--admin-border)] pb-4">
                    <div>
                        ...
                        <p className="text-xs text-[var(--admin-muted)] mt-0.5">
                            {entities.total} enregistrement{entities.total > 1 ? 's' : ''} répertorié{entities.total > 1 ? 's' : ''} dans la zone sélectionnée ({summary.zone.name})
                        </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2">
                        ...
                        <div className="flex items-center gap-1">
                            <button
                                ...
                                onClick={() => handlePageChange(entities.current_page - 1)}
