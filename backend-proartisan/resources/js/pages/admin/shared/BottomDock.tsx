...
export function BottomDock({ activeTab }: { activeTab: AdminTab }) {
    return (
        <div className="pointer-events-none fixed bottom-4 right-4 z-30 hidden lg:block">
            <div className="pointer-events-auto flex items-center gap-2 rounded-full border border-[var(--admin-border)] bg-[var(--admin-panel-strong)] p-2 shadow-[0_18px_38px_rgba(125,96,57,0.16)] backdrop-blur-xl">
                {quickDockTabs.map((tab) => (
                    <Link
                        ...
                        className={cn(
                            ...
                            activeTab === tab ? 'bg-[#f4e2bf] text-[#7d571b]' : 'text-[var(--admin-text-soft)] hover:brightness-110',
                        )}
                    >
                        {tabMeta[tab].label}
                    </Link>
