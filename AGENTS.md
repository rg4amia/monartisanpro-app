...
30. **Numéro de Paiement Mobile Money & Traçabilité des Reversements** : Les profils des acteurs prestataires (`artisan`, `livreur`, `fournisseur`) disposent des champs configurables `payment_phone` et `preferred_payment_provider` (`wave`, `orange_money`, `mtn_money`, `moov_money`). Ces coordonnées font autorité pour le virement automatisé des gains et déblocages de fonds (jalons MO, commissions de livraison, paiements matériaux). L'espace artisan fournit un accès One-Tap direct à l'historique complet et détaillé de ses paiements reçus avec rapprochement par mission, client et canal de paiement.
