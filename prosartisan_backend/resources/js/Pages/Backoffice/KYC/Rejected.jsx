...
export default function KYCRejected({ verifications, filters }) {
 const [search, setSearch] = useState(filters.search || '');

 const handleSearch = (e) => {
  e.preventDefault();
  router.get('/backoffice/kyc/rejected', { ...filters, search }, { preserveState: true });
 };
 ...
 return (
  <BackofficeLayout>
   <Head title="KYC - Rejetées" />

   <div className="mb-8">
    <div className="flex items-center justify-between">
     <div>
      <h1 className="text-3xl font-bold text-gray-900">Vérifications KYC Rejetées</h1>
      <p className="mt-2 text-sm text-gray-700">
       Consultez toutes les vérifications d'identité rejetées
      </p>
     </div>
      ...
      <tbody className="bg-white divide-y divide-gray-200">
       {verifications.data.map((verification) => (
        <tr key={verification.id} className="hover:bg-gray-50">
         ...
         <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
          {new Date(verification.updated_at).toLocaleDateString('fr-FR')}
         </td>
         <td className="px-6 py-4">
          <div className="text-sm text-gray-900 max-w-xs truncate">
           {verification.rejection_reason || 'Aucune raison spécifiée'}
          </div>
         </td>
         <td className="px-6 py-4 whitespace-nowrap">
   ...
   {verifications.data.length === 0 && (
    <div className="bg-white shadow rounded-lg p-6 text-center">
     ...
     <h3 className="mt-2 text-sm font-medium text-gray-900">Aucune vérification rejetée</h3>
     <p className="mt-1 text-sm text-gray-500">
      Aucune vérification KYC n'a été rejetée.
     </p>
    </div>
   )}
  </BackofficeLayout>
