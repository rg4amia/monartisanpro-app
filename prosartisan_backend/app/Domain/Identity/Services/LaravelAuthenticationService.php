...
class LaravelAuthenticationService implements AuthenticationService
{
 ...
 public function generateOTP(PhoneNumber $phone): OTP
 {
  try {
   ...
   $cacheKey = self::OTP_CACHE_PREFIX . $phone->toString();
   ...
   if ($this->smsService !== null) {
    ...
    Log::info("OTP generated for {$phone->toString()}: {$otp->getCode()}");
   }
 ...
 public function refreshToken(string $token): AuthToken
 {
  ...
  // Find user
  $user = $this->userRepository->findById(new \App\Domain\Identity\Models\ValueObjects\UserId($userId));
